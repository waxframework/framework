...
class Route
{
        ...
        rest_get_server()->register_route(
            $namespace, $full_route, [
                ...
                'permission_callback' => function() use( $middleware ) {
                    return static::permission_callback( $middleware );
                }
            ] 
        );
    }
    ...
    protected static function callback( $callback ) {
        return static::get_callback_response( $callback );
    }

    protected static function permission_callback( array $middleware ) {
        return true;
    }

    protected static function get_callback_response( $callback ) {
